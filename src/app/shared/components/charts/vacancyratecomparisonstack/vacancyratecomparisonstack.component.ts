...
@Component({
  selector: 'app-vacancyratecomparisonstack',
  templateUrl: './vacancyratecomparisonstack.component.html',
  styleUrls: ['./vacancyratecomparisonstack.component.css']
})
export class VacancyratecomparisonstackComponent implements OnInit {


  SpecificUseList: Array<any> = [];
  specificUseArray: any;
  masterChartdata4: any = [];
  chartdata4: any = [];
  ...
  displaySubLease4: boolean = true;

  chartHeader = "Vacancy Rate Comparison";
  yAxisLabel = "Vecancy Rate %";
  ...
  public onSearchClick() {
    ...
    let result = this._analyticsService.getStackVacancyRateComparison(dimentions);
    result.subscribe(item => {

      let data = JSON.parse(item['_body']).responseData.dashboardData[0];
      if (data) {
        this.masterChartdata4 = data;
        this.masterChartdata4.forEach(element => {
          element.IsSelected = true;
        });
        this.filterColumns4(null, null);
      }
    });
  ...
  filterColumns4(event: any, name:string){
    this.dataLoaded = false;
    this.customColors = [];
    this.chartdata4 = [];
    if(name)
    this.masterChartdata4.find(x => x.name == name).IsSelected = event.currentTarget.checked;
    if(this.displaySubLease4){
      this.masterChartdata4.filter(x => x.IsSelected == true).forEach(element => {
        this.chartdata4.push({IsSelected:true,name:element.name,"series":[{
  ...
  displaySubLeaseBars4(event: any) {
    this.displaySubLease4 =event.currentTarget.checked;
    this.filterColumns4(null, null);
  }

  applySubLeaseColors(){
    this.chartdata4.filter(x => x.Type == 'Sublease').forEach(y => {
  ...
  openNav4() {
    ...
    document.getElementById("chartNav4").style.width = "100%";
    document.getElementById("chartNavWrap4").style.width = "35%";
    if ($(window).width() < 767) {
      document.getElementById("chartNav4").style.width = "100%";
      document.getElementById("chartNavWrap4").style.width = "70%";
    }
    else {
      document.getElementById("chartNav4").style.width = "100%";
      document.getElementById("chartNavWrap4").style.width = "35%";
  ...
  closeNav() {
    ...
    document.getElementById("chartNav4").style.width = "100%";
    document.getElementById("chartNavWrap4").style.width = "0";
  }

  openChartNav() {
    this.closeNav();
    this.CloseActionBtnChkBox = true;
    document.getElementById("chartNavChkBox4").style.width = "100%";
    document.getElementById("chartNavWrapChkBox4").style.width = "35%";
    if ($(window).width() < 767) {
      document.getElementById("chartNavChkBox4").style.width = "100%";
      document.getElementById("chartNavWrapChkBox4").style.width = "70%";
    }
    else {
      document.getElementById("chartNavChkBox4").style.width = "100%";
      document.getElementById("chartNavWrapChkBox4").style.width = "35%";
  ...
  closeChartNav4() {
    ...
    document.getElementById("chartNavChkBox4").style.width = "100%";
    document.getElementById("chartNavWrapChkBox4").style.width = "0";
  }
